...
`index.html` 是單一大檔案，**必須使用錨點註解**讓未來 Agent 可精準定位，不需整包讀取。
...
### 未來維護方式
修改特定區塊時，用 Grep 搜尋錨點再 Read 該範圍，不需讀整個檔案：
```bash
grep -n "#SECTION:" index.html          # 列出所有錨點與行號
grep -n "#SECTION:PAGE-MEMORY" index.html  # 定位記憶翻牌的起始行
```
