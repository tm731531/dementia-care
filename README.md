...
**這個工具其實也在失智照護線上**——照顧失智媽媽要買醫療營養品（亞培安素、葡勝納 SR、倍速益、補體素、倍力素這類）。真正的兩個痛點是：

1. **各品牌成分表排序跟單位不統一**——有的先蛋白質、有的先熱量、單位 mg/mcg/per 份/per 100ml 混用，**客戶有需求站在貨架前要比，肉眼掃超容易漏行**。這個工具就是把差異並排對齊讓人不會看錯
2. **OCR 實務上不可行**——賣家放 100 張商品圖可能只有 10 張真的是成分表，剩下是行銷圖、情境照。營養標示字小又常印在曲面上。所以務實做法是**親自去家樂福/大樹拍包裝**，用 Claude/Gemini 辨識成結構化 JSON
...
目前 21 筆，以醫療營養品為主，也有嬰幼兒配方、高蛋白補充品、雞精等。
